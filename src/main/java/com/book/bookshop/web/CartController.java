...
public class CartController {
    ...
    public String list(HttpSession session, Model model) {
        User user = (User) session.getAttribute("user");
        List<CartVo> cartVos = cartService.findCartByUser(user.getId()); // 购物车列表展示的图书信息
...
//        userCartVo.setTotalPrice(cartService.getCartItemTotal(cartVos));
        UserCartVo userCartVo = cartService.wrapperCart(cartVos); // 代替上面三行，上面三行代码被封装
    ...
    public String update(HttpSession session, Cart cart) {
        cartService.updateById(cart); // 根据id来改
        User user = (User) session.getAttribute("user");
        List<CartVo> cartVos = cartService.findCartByUser(user.getId()); // 购物车列表展示的图书信息
